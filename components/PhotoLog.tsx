    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <h2 className="text-3xl font-semibold text-gray-800">Bitácora de Fotos</h2>
            ...
            <Modal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} title="Subir Nueva Foto">
                <div className="space-y-4">
                    ...
                    <input type="text" placeholder="Descripción" value={newPhoto.description || ''} onChange={e => setNewPhoto({...newPhoto, description: e.target.value})} className="w-full p-2 border rounded" />
                    <input type="text" placeholder="Etiquetas (separadas por coma)" onChange={e => setNewPhoto({...newPhoto, tags: e.target.value.split(',').map(t=>t.trim())})} className="w-full p-2 border rounded" />
            ...
            {selectedPhoto && (
                 <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title="Detalles de la Foto">
                    ...
                    <h3 className="text-lg font-semibold">{selectedPhoto.description}</h3>
                    <p className="text-sm text-gray-500 mb-2">Subido el: {new Date(selectedPhoto.uploadDate).toLocaleString()}</p>
                    <div className="flex flex-wrap gap-2">
                        {selectedPhoto.tags.map(tag => (
                            <span key={tag} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-semibold rounded-full">{tag}</span>
                        ))}
                    </div>
                 </Modal>
            )}
